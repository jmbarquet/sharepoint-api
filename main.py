...
def get_site_id(site_url: str, token: str) -> str:
    # site_url like "https://incrementa993.sharepoint.com"
    host = urllib.parse.urlparse(site_url).netloc
    resp = gget(f"https://graph.microsoft.com/v1.0/sites/{host}:/", token)
    return resp.json()["id"]

def get_folder_item(site_id: str, folder_path: str, token: str) -> dict:
    # folder_path like "Implemented Programs/InterContinental/IC Buenos Aires - 2016/Reportes/SR/2025"
...
# ---------- Data processing (matches your PQ logic) ----------
MONTH_MAP = {
    "jan":1,"ene":1,
    "feb":2,"fev":2,
    "mar":3,
...
def parse_month_from_filename(name: str) -> Optional[str]:
    ...
    if not mo: 
        return None
    return f"{MONTH_NAMES[mo-1]} {yy:02d}"
...
def process_bytes(name: str, content: bytes, sheet_name: str, skip_rows: int) -> pd.DataFrame:
    # Read sheet with skip_rows; raw header-less read
    ...
    # Expect columns B..G = indices 1..6 (Agent, Pos, Upsell, Nights, Sales, USD)
    sub = raw.iloc[:, 1:7].copy()
    ...
    sub = sub[sub["AgentRaw"] != ""]
    sub = sub[~sub["AgentRaw"].str.lower().isin(BAD_HEADERS)]
    sub = sub[sub["AgentRaw"].str.contains(r"[A-Za-zÀ-ÿ]", regex=True)]
    ...
    # bring Agent display (first form seen)
...
def commissions(
    site_url: str = Query("https://incrementa993.sharepoint.com"),
    folder_path: str = Query(..., description="e.g. Implemented Programs/InterContinental/IC Buenos Aires - 2016/Reportes/SR/2025"),
    sheet_name: str = Query("Comisiones"),
    skip_rows: int = Query(7, ge=0, le=100)
):
    token = get_access_token()
    site_id = get_site_id(site_url, token)

    # allow callers to omit "Implemented Programs/" — we try both
    try_paths = {folder_path.strip()}
    try_paths.add(f"Implemented Programs/{folder_path.strip()}") if not folder_path.lower().startswith("implemented programs/") else None
    ...
    # rename for JSON keys Bubble-friendly (no spaces)
